...
where
\begin{itemize}
	\item $M$ is an integer such that $|M| \le B^{p}-1$. It is called the \textit{integral siginificand} of x \tristan{or the mantissa?}.
...
ignore the most significand bit without losing precision. \tristan{Add a sentence to explain what it is.}
...
floating-point numbers $a$ and $b$, such that $a < x < b$ and $a \neq b$ \tristan{It's unclear what 0.5 ulp means from this definition}.
...
Since the beginning of computers, there have been multiple proposed solutions to represent floating-point numbers \tristan{I think ``floating-point numbers'' should be used instead of ``floating-points''}.
...
For example, it was found that some convolution neural network (CNN) perform well with binary16; and even with precision of only 8 bits \tristan{reference?}.
