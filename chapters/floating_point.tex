...
In radix 2, the first digits of the siginificand is 1 for normal numbers and 0 for subnormal ones.
\HL{
	...
	A nubmer that is too large to be represented by a data format precision and exponent range is said to \textit{overflow}.
	Similarly, when a number is too close to zero and cannot be represented, it \textit{underflow}.
	The availability of subnormal numbers allows for \textit{gradual underflow},
	which in case of undeflow would return an increasingly smaller subnormal value instead of zero.
	Mitigating the precision loss from underflows helps in implementing numerically stable code.
	Round-off error accumulation occurs when a series of arithmetic operation amplify
	an initially small error into a much larger one; the algorithm is said to be \textit{unstable}.
	Adding a large number and a small number requires shifting the mantissa bits 
	of the smaller number to the right resulting in lost of precision for the 
	smaller number; this is known as \textit{swamping}~{\cite{SwampCancel}}.
	...
	be affected by rounding errors, thus carrying more error into the subtraction.
}
...
\HL{
	When performing arithmetic computation in high-precision register, the 
...
\HL{When correctly rounded, arithmetic operations remain commutative with addition and multiplication.}
...
\HL{
	The IEEE~754 format requires the following arithmetic functions are required to 
	be correctly rounded: addition, subtraction, multiplication, division, and fused multiply-add (FMA).
	Moreover, the square root function and conversion between supported formats also need to be correctly rounded.
}
